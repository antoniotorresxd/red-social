    ...
    <div v-if="show" class="publish-overlay" :style="overlayStyles" @click.self="$emit('close')">
      <div class="publish-card" style="max-width:380px;">
        <h5 class="mb-3 text-center">
          ¿Está seguro de salir del {{ communityType === 'group' ? 'grupo' : 'foro' }}?
        </h5>
        <div class="d-flex justify-content-between mt-4">
          <BButton variant="secondary" @click="$emit('close')" :disabled="isExiting">
            Cancelar
          </BButton>
          <BButton variant="danger" @click="$emit('exit')" :disabled="isExiting">
