<template>
  <div class="publications-wrapper">
    <div class="publications-area" ref="root">
      <div
        class="d-flex justify-content-between align-items-center mb-3 pub-header"
        ref="header"
      >
        <button
          v-if="showTaskSubmissions"
      ...
      <div>
        ...
        <template v-else>
          ...
          >
            <div v-if="isLoadingPubs" class="text-center py-3">
              Cargando publicaciones…
            </div>
            <div v-else-if="errorPubs" class="text-danger text-center py-3">
              {{ errorPubs }}
            </div>
    ...
    <TaskUploadModal
      ...
      :task="selectedTask"
      :overlayStyles="uploadOverlayStyles"
      @close="closeUploadModal"
      @uploaded="fetchPublications"
    />

    <MembersModal
      :show="showMembers"
      :overlayStyles="membersOverlayStyles"
      :members="members"
    ...
    <DeleteCommunityModal
      ...
      :overlayStyles="deleteOverlayStyles"
      :isDeleting="isDeleting"
      :error="deleteError"
      :communityType="selected.type"
      @close="closeDeleteModal"
      @delete="confirmDeleteCommunity"
    />

    <ExitCommunityModal
      :show="showExitModal"
      :overlayStyles="exitOverlayStyles"
      :isExiting="isExiting"
      :error="exitError"
      :communityType="selected.type"
      @close="closeExitModal"
      @exit="confirmExitCommunity"
    />
    ...
    <teleport to="body">
      <div
        v-if="modalType === 'task'"
        class="publish-overlay mt-5 pt-5"
        :style="modalOverlayStyles"
        @click.self="closeModal"
      >
        <div class="publish-card">
  ...
  methods: {
    showGroupCode() {
      const noti = {
        ...
        title: `Grupo - ${this.selected.name}`,
        message: `El código del grupo es: ${this.selected.code}`,
        type: "all",
    ...
    setupKebab() {
      ...
      if (this.isAdmin) {
        ...
        if (this.selected.type === "group") {
          ...
          this.kebabItems.push("Código"); // <-- AGREGAR AQUÍ
        }
    ...
    async confirmExitCommunity() {
      ...
      try {
        ...
        this.exitError =
          err.response?.data?.message || "No se pudo salir de la comunidad.";
      } finally {
